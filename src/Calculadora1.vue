...
<template>
    ...
    <table>
      ...
      <tbody v-for="(scope_carrito, scope) in carrito_scoped" :key="scope">
        ...
        <tr v-for="(ic, icid) in scope_carrito" :key="icid">
          ...
          <td> {{ formatDecimals(calculo(get_ghg_value(ic.id, ic.ghg, ic.year), ic.value)) }} </td>
          <td><button @click="EliminarCampo(icid as string)">BORRAR</button></td>
        </tr>
        <tr>
          <th colspan="4">Total {{ scope }}</th>
          <td>{{ formatDecimals(fixDecimals(Object.values(scope_carrito).map((ic) => calculo(get_ghg_value(ic.id,
          ic.ghg, ic.year),
          ic.value)).reduce((a, b) => a + b, 0))) }}</td>
          <td></td>
        </tr>
      </tbody>
      <tfoot>
        <tr>
          ...
          <td>
            {{
  formatDecimals(fixDecimals(Object.values(carrito).map((ic) => calculo(get_ghg_value(ic.id, ic.ghg, ic.year),
    ic.value)).reduce((a, b) => a + b, 0)))
            }}
          </td>
          <td style="border-bottom: 0 !important; border-right: 0;">
  ...
  <div>
    ...
    <div>
      ...
      <input class="form-control" type="text" v-model="description" placeholder="Introduce una descripción" />

      <ul class="list-group">

        <li class="list-group-item d-flex justify-content-between align-items-center"
          v-for="(ghgValue, ghgKey) in valuesGhg" :key="ghgKey">

          <b>{{ ghgKey }}</b>: {{ formatDecimals(calculo(ghgValue, parseFloat(value.toString()))) }}
          <button type="button" class="btn btn-info" @click="anadir(currentItem, ghgKey.toString())"> Seleccionar
          </button>
