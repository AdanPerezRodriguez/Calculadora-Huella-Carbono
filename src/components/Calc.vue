...
const valuesGhg = computed(() => {
  ...
  if (typeof datos != 'undefined') {
    ghgs = datos.value?.GHG[menuItem.data_year] || {};
  }
...
const data_base_filtered = computed(() => {
  ...
  if (recursive_year) {
    ...
    filt_data = filt_data.map((r) => {
      let y = parseInt(menuItem.data_year);
...
//* Save this data in cartItems.ts store?
...
<template>
  ...
  <div class="container p-3">
    <div class="d-flex">
      ...
      >
        <label
          v-if="menuItem.lang.toLocaleUpperCase() == 'UK'"
